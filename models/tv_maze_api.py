...
class TVMazeAPI(object):
    ...
    def fetch(show_name=None, tvmaze_id=None):
        ...
        show = Show(
            thetvdb_id=tvm_show.externals
            and tvm_show.externals.get("thetvdb"),
            ...
            title=tvm_show.name,
            slug=sa_helper.generate_slug(sa_session, Show,
                                         slugify.slugify(tvm_show.name)),
            ...
            schedule_days=' '.join(
                tvm_show.schedule and tvm_show.schedule.get("days") or []),
            ...
            tvmaze_updated_at=datetime.fromtimestamp(tvm_show.updated,
        ...
        for tvm_episode in tvm_show.episodes:
            episode = Episode(
                ...
                and parse(tvm_episode.airstamp).astimezone(utc).replace(
                    tzinfo=None),
        ...
        for tvm_genre in tvm_show.genres:
            genre, was_created = sa_helper.get_or_create(
                sa_session, Genre, name=tvm_genre)
            sa_helper.get_or_create(
    ...
    def refresh(show, tvmaze_id):
        ...
        show.thetvdb_id = tvm_show.externals and tvm_show.externals.get(
            "thetvdb")
        show.tvrage_id = tvm_show.externals and tvm_show.externals.get(
            "tvrage")
        ...
        show.schedule_days = ' '.join(
            tvm_show.schedule and tvm_show.schedule.get("days") or [])
        show.schedule_time = tvm_show.schedule and tvm_show.schedule.get(
            "time")
        show.status = tvm_show.status
        show.tvmaze_url = tvm_show.url
        show.last_cached_at = datetime.utcnow()
        show.tvmaze_updated_at = datetime.fromtimestamp(tvm_show.updated,
                                                        utc).replace(
                                                            tzinfo=None)
    ...
    def sync_cache(re_cache_all=False):
        ...
        show_objects = sa_session.query(Show).filter(
            Show.tvmaze_id.in_(tvm_updates.keys()))
