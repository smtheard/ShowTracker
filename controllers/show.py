...
def shows_by_page(session, page):
    ...
    shows = sa_session.query(Show).outerjoin(ShowFollow).group_by(
        Show.id).order_by(func.count(ShowFollow.id).desc()).order_by(
            Show.tvmaze_updated_at.desc()).limit(12).offset(int(page) * 12)

    return dict(
        shows=[show.to_card_dict(user=user) for show in shows],
        success=True,
...
def shows_query(session, page):
    ...
    page_count = math.ceil(
        sa_session.query(Show).filter(
            Show.title.ilike('%' + query + '%')).count() / 12)
    shows = sa_session.query(Show).filter(
        Show.title.ilike('%' + query + '%')).limit(12).offset(int(page) * 12)
